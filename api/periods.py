...
def get_period_rankings(period: str = Query("daily"), limit: int = Query(30)):
    ...
    try:
        row = conn.execute(
            "SELECT MAX(ranking_date) AS d FROM daily_rankings WHERE source='oliveyoung'"
        ).fetchone()
        ...
        base_row = conn.execute(
            ...
            "WHERE source='oliveyoung' AND ranking_date <= ?", (target,)
        ).fetchone()
        base = base_row["d"] if base_row and base_row["d"] else None

        # 🚨 데이터 부족 판정: 주간/월간은 충분한 히스토리 필요
        insufficient = False
        if period != "daily":
            if base is None:
                insufficient = True
        ...
        cur = conn.execute(
            """SELECT r.rank_num, r.product_id, p.brand, p.product_name, p.product_url,
               ...
               WHERE r.source='oliveyoung' AND r.ranking_date=?
        ...
        if base and not insufficient:
            for b in conn.execute(
                ...
                "WHERE source='oliveyoung' AND ranking_date=?", (base,)
            ).fetchall():
